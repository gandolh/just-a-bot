...
import { RollingView } from './RollingView.tsx';
...
export function Game({ client, me, conn }: Props) {
  ...
  if (conn.kind === 'engine-offline') {
    return (
      <main className="shell">
        ...
        <p className="tag">The bot has dropped its connection; your table will resume when it returns.</p>
      </main>
    );
  }
  ...
  switch (state.phase) {
    ...
    case 'rolling': return <RollingView me={me} state={state} />;
    case 'finished': return <ResultView me={me} state={state} />;
  }
}
